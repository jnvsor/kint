...
abstract class Kint_Renderer_RichVue_Plugin
{
    protected $renderer;

    public function __construct(Kint_Renderer_RichVue $r)
    {
        $this->renderer = $r;
    }
    ...
    public function renderHeaderLocked(Kint_Object $o, $content)
    {
        ...
        if (Kint_Renderer_RichVue::$access_paths && $o->depth > 0 && $ap = $o->getAccessPath()) {
    ...
    public static function renderLockedHeader(Kint_Object $o, $content)
    {
        ...
        if ($show_dep) {
            trigger_error('Kint_Renderer_RichVue_Plugin::renderLockedHeader() is deprecated and will be removed in Kint 3.0. Use renderHeaderLocked instead.', KINT_PHP53 ? E_USER_DEPRECATED : E_USER_NOTICE);
            $show_dep = false;
        }
